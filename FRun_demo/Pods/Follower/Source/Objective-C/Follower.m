...
- (void)endRouteTracking {
    ...
    NSString *distance = [NSString stringWithFormat:@"%f",self.totalDistance];
    [ZJStoreDefaults setObject:distance forKey:@"todaydistance"];
    
    NSString *totaldis = [ZJStoreDefaults getObjectForKey:@"totaldistance"];
    
    if (totaldis) {
        NSString *totaldistance = [NSString stringWithFormat:@"%f",([totaldis doubleValue]+self.totalDistance)/1000];
        [ZJStoreDefaults setObject:totaldistance forKey:@"totaldistance"];
    }else{
        [ZJStoreDefaults setObject:distance forKey:@"totaldistance"];
    }
...
- (void)handleLocationUpdate:(CLLocation *)location {
    ...
    // Coordinates
    //
    CLLocationCoordinate2D coord = [JZLocationConverter wgs84ToGcj02:location.coordinate];
...
//    _routeRegion = MKCoordinateRegionMake(centerCoordinate, span);
    _routeRegion = MKCoordinateRegionMakeWithDistance(centerCoordinate, 1500, 1500);

//    _routeRegion = MKCoordinateRegionMakeWithDistance(centerCoordinate,1000, 1000);
}
...
- (void)createPolylineForRoute {
    ...
    for (NSUInteger i = 0; i < locationsCount; i++) {
        ...
        coords[i] = [JZLocationConverter wgs84ToGcj02:location.coordinate];
    }
...
- (CLLocationSpeed)averageSpeedWithUnit:(SpeedUnit)speedUnit {
    ...
    if (self.totalDistance != 0) {
        averageSpeed = self.totalDistance / self.routeDuration;
    }else{
        averageSpeed = 0;
    }
