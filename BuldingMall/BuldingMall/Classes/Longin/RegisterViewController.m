...
-(void)loginclick{
    //猪猪修改，添加判断
    if (![ZJStaticFunction isMobileNumber:_numText.text]) {
        [ZJStaticFunction alertView:self.view msg:@"请输入正确的手机号"];
        return;
    }
    if (_passText.text.length != 5){
        [ZJStaticFunction alertView:self.view msg:@"请输入正确的验证码"];
        return;
    }
    if (_pwtext.text.length < 6 || _pwtext.text.length > 20) {
        [ZJStaticFunction alertView:self.view msg:@"请输入6-20位密码"];
        return;
    }
    
    if (![_pwtext.text isEqualToString: _pwagtext.text]) {
        [ZJStaticFunction alertView:self.view msg:@"两次密码输入不相同"];
        return;
    }
    
    NSDictionary *dic = @{@"mobile":_numText.text,@"code":_passText.text,@"password":_pwtext.text};
    [HTTPRequest postWithURL:@"register" params:dic ProgressHUD:_HUD controller:self response:^(id json) {
        NSLog(@"json:%@",json);
