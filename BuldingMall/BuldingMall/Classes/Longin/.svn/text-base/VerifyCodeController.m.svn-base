...
@interface VerifyCodeController ()<UITextFieldDelegate,UIAlertViewDelegate>
{
    UITextField *yantf;
    UIButton *yanbtn;
    UIButton *yuyi;
...
- (void)viewDidLoad {
    ...
    if (self.regisType == VTyperRegister) {
        self.title = @"快速注册";
    }else if (self.regisType == VTypeForgotPassword){
        self.title = @"密码找回";
    }
...
-(void)getverifdfd
{
    if ([ZJStaticFunction isMobileNumber:self.telname]==YES)
    {
        ...
        if (self.regisType == VTyperRegister) {
            NSDictionary *dic = @{@"mobile":self.telname,@"yytype":@"0"};
...
//    NSString *yanma = [ZJStaticFunction trimSpaceInTextField:yantf.text];
    ...
    if ([yantf.text isEqualToString:_getCodereturn])
    {
        ...
        setting.code = yantf.text;
        
        if (self.regisType == VTyperRegister) {
            setting.regisType = LTyperRegister;
        }else if (self.regisType == VTypeForgotPassword){
            setting.regisType = LTypeForgotPassword;
        }
