...
- (void)startEncodeDataWithPixelBuffer:(CVPixelBufferRef)buffer
                         withTimeStamp:(CMTime) time
                      isNeedFreeBuffer:(BOOL)isNeedFreeBuffer{
    [self startEncodeWithPixelBuffer:buffer withTime:time session:mSession isNeedFreeBuffer:isNeedFreeBuffer isDrop:NO needForceInsertKeyFrame:self.needForceInsertKeyFrame lock:self.lock];
}
...
-(void)startEncodeWithPixelBuffer:(CVPixelBufferRef)buffer withTime:(CMTime)time session:(VTCompressionSessionRef)session isNeedFreeBuffer:(BOOL)isNeedFreeBuffer isDrop:(BOOL)isDrop  needForceInsertKeyFrame:(BOOL)needForceInsertKeyFrame lock:(NSLock *)lock{
    ...
    CMTime presentationTimeStamp = time;
    
    // Switch different source data will show mosaic because timestamp not sync.
    static int64_t lastPts = 0;
    int64_t currentPts = (int64_t)(CMTimeGetSeconds(time) * 1000);
