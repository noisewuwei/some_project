...
class DrawerFromRightAnimation: BaseAnimation {
    ...
    override func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if isPresenting {
            ...
            let showFrame = CGRect(x: size.width-toViewSize.width, y: 0, width: toViewSize.width, height: toViewSize.height)
            let hiddenFrame = CGRect(x: size.width, y: 0, width: toViewSize.width, height: toViewSize.height)
            ...
            let hiddenFrame = CGRect(x: size.width, y: 0, width: fromViewSize.width, height: fromViewSize.height)
            UIView.animate(withDuration: transitionDuration(using: transitionContext)) {
