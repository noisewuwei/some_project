...
class SignalController: BaseViewController {
    ...
    init(signalClient: SignalingClient, webRTCClient: WebRTCClient) {
        ...
        self.signalClient.stream_message_p2pMessageCallback = { [weak self] data in
            
            let array = data as! Array<Any>
            let dic = JSON(array.first as Any)
            let msgDic = dic["msg"].dictionaryValue
            
            self?.peerSocket = msgDic["peerSocket"]?.stringValue ?? ""
            self?.streamId = msgDic["streamId"]?.intValue ?? 0
...
extension SignalController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        switch item {
        case .socketConnect:
            self.signalClient.sendStr(str: "xixixixi")
        case .sendOffer:
        ...
        case .sendAnswer:
            self.webRTCClient.answer { [weak self] (localSdp) in
                self?.hasLocalSdp = true
                let msg = ["type":"answer","sdp":localSdp.sdp,"receivedSessionVersion":2,"config":["maxVideoBW":100000]] as [String : Any]
                let options = ["streamId":self?.streamId,"peerSocket":self?.peerSocket,"msg":msg] as [String : Any]
                self?.signalClient.sendTypeMessage(message: ["options":options], type: .streamMessageP2P)
            }
        case .video:
...
extension SignalController: WebRTCClientDelegate {
    ...
    func webRTCClient(_ client: WebRTCClient, didDiscoverLocalCandidate candidate: RTCIceCandidate) {
        ...
        let msg = ["type":"candidate","candidate":["sdpMLineIndex":0,"sdpMid":"0","candidate":candidate.sdp],"receivedSessionVersion":2] as [String : Any]
        let options = ["streamId":self.streamId,"peerSocket":self.peerSocket,"msg":msg] as [String : Any]
        self.signalClient.sendTypeMessage(message: ["options":options], type: .streamMessageP2P)
